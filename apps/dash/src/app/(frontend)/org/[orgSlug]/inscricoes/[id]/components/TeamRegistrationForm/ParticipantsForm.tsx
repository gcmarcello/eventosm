    ...
    fields.forEach((field, index) => {
      if (form.getValues(`teamMembers.${index}.selected`)) {
        ...
        form.setValue(
          ...
          filterCategories(
            event.EventModality.find((mod) => mod.id === modalityId)
              ?.modalityCategory || [],
            {
      ...
      {form.watch("teamMembers").length ? (
        <div className="mx-4 mb-4 flex flex-col items-center justify-end gap-3 lg:flex-row lg:justify-center lg:gap-10">
          <div className="max-w-[500px]">
            ...
            <Button
              disabled={event.EventModality.length < 2}
              className={"mt-4"}
              onClick={() => {
                      ...
                      >
                        <TableCell>
                          ...
                          >
                            <Checkbox
                              ...
                              onChange={(e) => {
                                if (!e) {
                                  if (event.EventModality.length > 1)
                                    form.resetField(
                                      `teamMembers.${index}.modalityId`
                                    );
                                  form.resetField(
                        ...
                        <TableCell>
                          {event.EventModality.length > 1 ? (
                            ...
                            <Text>
                              Modalidade Única - {event.EventModality[0]?.name}
                            </Text>
                          )}
                        </TableCell>
